...
- **位置**：`Assets/Game/Scripts/Data/`、`Assets/Game/Scripts/Network/`、`Assets/Game/Scripts/UI/`
- **核心管理器**：
  - `Data.Instance`：数据管理器
  - `Net.Instance`：网络管理器
  - `UI.Instance`：UI管理器
...
- **核心功能**：
  ...
  - 心跳机制：Ping/Pong 保活
  - 协议路由：将消息分发到对应处理逻辑
- **文件**：`Assets/Game/Scripts/Network/Net.cs`
...
- **文件**：`Assets/Game/Scripts/UI/`（待补充具体实现）
...
- **核心功能**：
  ...
  - 背景音乐：音乐播放、切换
  - 音量控制：音效/音乐音量调节
- **文件**：`Assets/Game/Scripts/Data/Audio.cs`
...
2. **Event 全局事件总线**：跨模块通信
   ```csharp
   Game.Event.Instance.Add(eventKey, callback);
   Game.Event.Instance.Fire(eventKey, args);
   ```
