...
- **Entry Point（1 个）**：`Launcher/` 无独立 `.asmdef`，编译进 Unity 默认的 `Assembly-CSharp`。依赖 Framework 完成设备认证、热更新检查、程序集加载，最终反射调用 `Game.Start.Gate.Entrance()` 启动游戏。无任何程序集反向引用 Launcher。
