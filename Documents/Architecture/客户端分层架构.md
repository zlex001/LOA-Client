...
graph TB
    ...
    subgraph hotUpdate ["Hot Update Assemblies — 7 .asmdef"]
        Start[Start]
...
- **Hot Update（7 个）**：每层一个 `.asmdef`，支持运行时增量更新。依赖方向从高层流向低层，反向通信使用事件机制。
- **AOT（4 个）**：编译进安装包。`Framework` 被全部 7 个热更新程序集引用，提供 `Singleton<T>`、`AssetManager`、`Http`、`Hot`、`Localization` 等基础服务；3 个第三方库因体积大且稳定，不纳入热更新。
...
| `HotScript/` | 热更新 | 游戏代码（7 个热更新程序集 + ThirdParty），详见[第二章](#二各层详细说明) |
...
{
    "name": "Start",
    "references": ["Framework", "Basic", "Data", "Net", "Presentation"]
}
```
...
LOA客户端采用**HybridCLR多程序集热更新**方案，将游戏代码拆分为7个独立的热更新程序集：
...
2. 在`Hot Update Assembly Definitions`中添加7个asmdef文件
3. 保存设置
...
| 程序集数量 | 1个（默认） | 10个（7个热更新+3个AOT） |
| 热更新程序集 | 1个（Game） | 7个（分层独立） |
| 热更新粒度 | 全量更新 | 增量更新 |
| 依赖管理 | 无约束 | .asmdef强约束 |
| 与服务端一致性 | 低 | 高 |
| 可维护性 | 中 | 高 |
