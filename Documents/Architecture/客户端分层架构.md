...
graph TB
    ...
    subgraph hotUpdate ["Hot Update Assemblies — 8 .asmdef"]
        Start[Start]
...
项目共 **13 个程序集**，按更新机制分为三类：

- **Entry Point（1 个）**：`Launcher/` 无独立 `.asmdef`，编译进 Unity 默认的 `Assembly-CSharp`。依赖 Framework 完成设备认证、热更新检查、程序集加载，最终反射调用 `Game.Gate.Entrance()` 启动游戏。无任何程序集反向引用 Launcher。
- **Hot Update（8 个）**：每层一个 `.asmdef`，支持运行时增量更新。依赖方向从高层流向低层，反向通信使用事件机制。
- **AOT（4 个）**：编译进安装包。`Framework` 被全部 8 个热更新程序集引用，提供 `Singleton<T>`、`AssetManager`、`Http`、`Hot`、`Localization` 等基础服务；3 个第三方库因体积大且稳定，不纳入热更新。

> **Launcher 与 Framework 为何分离？** Framework 是**库**——被 8 个热更新程序集通过 `.asmdef` 显式引用，必须有独立程序集名称。Launcher 是**入口点**——无人引用它，留在 `Assembly-CSharp` 即可。二者依赖方向相反，职责不同，不应合并。
...
| `Game/Scripts/` | 热更新 | 游戏代码（8 个热更新程序集 + ThirdParty），详见[第二章](#二各层详细说明) |
