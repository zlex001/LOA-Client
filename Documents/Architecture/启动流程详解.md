...
**位置**：`Assets/HotScript/Presentation/Gate.cs`（热更新代码）
...
flowchart TD
    ...
    InitNet --> RequestGW[请求服务器列表<br/>Gateway.Request]
...
4. **资源下载**：下载DLL和资源（按需）
5. **程序集加载**：加载热更新代码
6. **热更新入口**：初始化管理器 + 发起 Gateway 请求

整个流程通过 **Main.cs → Hot.cs → Gate.cs（Presentation层）→ 管理器层** 完成从Unity启动到进入游戏的完整过渡。
