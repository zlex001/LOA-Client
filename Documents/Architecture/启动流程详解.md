...
**位置**：`Assets/HotScript/Basic/Gate.cs`（热更新代码）
...
public static void Entrance(string gateway)
{
    ...
    Authentication.Init();     // 5. 登录逻辑（注册 DataManager 事件监听）
...
**职责**：
1. 设置网关地址和语言
2. 按顺序初始化所有管理器（含 `Authentication.Init()` 注册登录事件监听）
3. 启动游戏流程状态机
...
**位置**：`Assets/HotScript/Basic/Gate.cs`
...
flowchart TD
    ...
    InitNet --> InitAuth[Authentication.Init]
    InitAuth --> StartFlow[启动流程状态机<br/>StartupFlowManager.Start]
