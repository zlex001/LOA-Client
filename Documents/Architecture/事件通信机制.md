...
**位置**：`Assets/Game/Scripts/Basic/Monitor.cs`
...
**Event** 是全局事件总线，用于跨模块通信。所有模块可以通过 `Game.Event.Instance` 访问。

**位置**：`Assets/Game/Scripts/Basic/Event.cs`
...
button.onClick.AddListener(() => {
    ...
    Game.Event.Instance.Fire(Start.Event.ServerSelect, selectedServer);
});
```
...
void OnDestroy()
{
    ...
    Game.Event.Instance.Remove(Start.Event.ServerSelect, OnStartServerSelect);
}
```
...
// Data层
Game.Event.Instance.Add(UI.Event.Click, (args) => {
    Audio.Instance.Play(Config.Audio.Click);
});
```
...
// 不推荐：使用字符串字面量
Game.Event.Instance.Fire("click");  // ❌ 容易拼写错误
```
...
private void OnDataChanged(params object[] args)
{
    for (int i = 0; i < 100; i++)
    {
        Game.Event.Instance.Fire(SomeEvent, i);  // 触发100次事件
    }
}
...
private void OnDataChanged(params object[] args)
{
    ...
    Game.Event.Instance.Fire(SomeEvent, results);  // 触发一次
}
```
...
for (int i = 0; i < 100; i++)
{
    Game.Event.Instance.Add($"Event{i}", handler);
}

// ✅ 推荐：合并事件
Game.Event.Instance.Add("BatchEvent", (args) => {
