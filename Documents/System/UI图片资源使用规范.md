...
基于项目全量 Prefab 和 C# 动态创建代码的实际数据提炼，按背景色场景分类：

#### 深色背景（游戏内 UI）

覆盖 Home、Option、Story、Login、Initialize 等所有 Prefab 体系 UI。

| 角色 | RGBA | 说明 |
|------|------|------|
| **正文/标题/按钮文字** | `(1, 1, 1, 1)` 白色 | 项目中所有 Prefab 的标准文字颜色 |
...
覆盖 StartSettings 等浅色面板。

| 角色 | RGBA | 说明 |
|------|------|------|
| **主文本** | `(0, 0, 0, 1)` 黑色 | 标题、标签、输入框文字 |
| **次要文本** | `(0.557, 0.557, 0.576, 1)` 灰色 | 设置项值、辅助说明 |
| **分区标题** | `(0.5, 0.5, 0.52, 1)` 深灰 | Section Header |
| **强调/链接** | `(0.2, 0.5, 0.9, 1)` 蓝色 | 添加按钮等操作文字 |
| **按钮文字（深色按钮上）** | `(1, 1, 1, 1)` 白色 | 确认/删除等有色背景按钮 |
| **按钮文字（浅色按钮上）** | `(0, 0, 0, 1)` 黑色 | 取消等浅灰背景按钮 |
| **输入框占位符** | `(0.5, 0.5, 0.5, 0.5)` 半透灰 | InputField Placeholder |
