...
flowchart TD
    ...
    Gateway --> CheckLocal{本地有账号?}
    
    CheckLocal -->|无账号| FirstTimeUI[首次启动界面]
    FirstTimeUI --> BigButton[显示：立即游玩]
    BigButton --> QuickStart[QuickStart请求]
...
1. **检测本地账号**
   - 读取`UserData.pb`，检查`Accounts`列表
   - 如果为空，判定为首次启动

2. **显示首次启动界面**
   - 只显示：游戏Logo、服务器选择、"立即游玩"按钮
   - 不显示：账号列表、Login按钮、账号管理功能
...
1. **检测本地账号**
   - 读取`UserData.pb`，存在账号
   - 取`User.SelectedAccountIndex`对应的账号

2. **自动登录（不显示Start界面）**
   - 显示"连接中..."过渡界面（Dark UI）
   - 连接游戏服务器
   - 发送`Login`协议（包含accountId, password）
