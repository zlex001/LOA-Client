...
**文件**：
- `Assets/HotScript/Presentation/Start.cs` - 设置按钮初始化和布局
- `Assets/HotScript/Presentation/StartSettings.cs` - 设置面板实现
- `Assets/HotScript/Data/Config.cs` - UI 配置定义
...
**核心原则**：只有在**网络完全不通时仍需显示的文本**才需要客户端本地化

**判断标准**：
> 这个文本显示时，客户端是否有任何机会从服务器获取过文本？

### 技术必需的5个键

启动阶段的网络错误提示，这些是客户端本地化的**技术必然性**：

| 键名 | 中文文本 | 使用位置 | 必需理由 |
|------|---------|---------|---------|
| `loading` | 载入中... | Gate.cs:61 | 启动时尚未连接网关服务器 |
| `connecting` | 连接中... | Start.cs:289 | 点击登录尚未建立Socket连接 |
...
**以下文本理论上可以由服务器推送（SDUI架构），但当前实现选择了客户端本地化**：

1. **游戏中网络错误**（11个）
   - `server_disconnected`, `send_failed`, `reconnecting_countdown` 等
   - 显示时用户已登录游戏，服务器可以在登录响应中推送这些文本
   - 当前选择客户端本地化，简化服务器逻辑

2. **热更新阶段文本**（13个，预留未使用）
   - `downloading_resources`, `unpacking_resources` 等
   - 显示时已连上网关，可以由网关推送
   - 当前预留在客户端，为未来热更新功能准备

3. **StartSettings界面**（30个）
   - `start_settings_accounts`, `start_settings_language` 等
   - 能打开此界面说明之前已连上服务器，可以由服务器推送
   - 当前选择客户端本地化，原因：
     - 架构清晰（纯客户端界面独立于服务器）
     - 开发便利（不需要服务器推送配置界面文本）

4. **语言名称**（22个）
   - `lang_ChineseSimplified`, `lang_Japanese` 等
...
| 场景 | 推荐方案 | 理由 |
|------|---------|------|
| 网关连接失败 | 客户端本地化 | 技术必需（还未连上服务器） |
| 游戏中断网 | 服务器推送（SDUI） | 登录时可推送，或客户端本地化简化实现 |
| StartSettings界面 | 客户端本地化 | 架构清晰（纯客户端界面） |
| Start/Home/Option界面 | 服务器推送（SDUI） | 灵活性（运营文案、A/B测试） |

**当前状态**：混合架构
- 技术必需的5个键：客户端本地化 ✅
- 游戏中网络错误：客户端本地化（实现选择）
- StartSettings：客户端本地化（实现选择）
- Start/Home/Option/Initialize/Tutorial：SDUI ✅
...
**当前实现**（`Assets/HotScript/Presentation/Start.cs`）：
...
**当前流程**（`Assets/HotScript/Basic/Gate.cs`）：
...
**当前实现**（`Assets/HotScript/Presentation/Start.cs:319-333`）：
...
**当前Net.cs实现**（`Assets/HotScript/Net/Net.cs:506-516`）：
